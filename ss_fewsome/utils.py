...
def get_pseudo_labels(train_ids_path, path_to_anom_scores, data_path, margin, metric, current_epoch, num_pseudo_labels, model_name):
    ...
    if isinstance(current_epoch, dict):
        ...
        for key in current_epoch.keys():
            ...
            f = [f for f in files_total if ( ('epoch_' + str(current_epoch[key]) in f) &  ('seed_' + str(key) in f)  &   ('on_test_set_' not in f)  ) ][0]
            files.append(f)

    else:
        files = [f for f in files_total if ( ('epoch_' + str(current_epoch) in f) &  ('on_test_set_' not in f)  ) ]
        assert len(files) == 10
...
def print_ensemble_results(path_to_anom_scores, epoch, stage, metric, meta_data_dir):
    ...
    if isinstance(epoch, dict):
        ...
        for key in epoch.keys():
            files = files + [file for file in files_total if (('epoch_' + str(epoch[key]) ) in file) & ('on_test_set' not in file ) & ('seed_' + str(key) in file) ]
    ...
    if isinstance(epoch, dict):
        files=[]
        for key in epoch.keys():
            files = files + [file for file in files_total if (('epoch_' + str(epoch[key]) ) in file) & ('on_test_set' in file ) & ('seed_' + str(key) in file)]
    else:
...
def combine_results(path_to_anom_scores_oa, path_to_anom_scores_sev, epoch_oa, epoch_sev, metric, meta_data_dir):
    ...
    for key in epoch_oa.keys():
        files = files + [file for file in files_total if (('epoch_' + str(epoch_oa[key]) ) in file) & ('on_test_set' in file ) & ('seed_' + str(key) in file) ]
    ...
    print('---------------------------------------------------- For stage ' + stage + '----------------------------------------------------')
    print('-----------------------------RESULTS ON TEST SET---------------------------')
    ensemble_results(df_oa, stage, 'comb_score', meta_data_dir)
