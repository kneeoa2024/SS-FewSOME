...
def dclr_training(args, model_temp_name_stage, stage, pseudo_label_ids, epochs, num_ss, current_epoch, self_supervised = 1, semi= 0, seed=None):
...
def parse_arguments():
    ...
    parser.add_argument('--start_margin', type=float, default = 1.15)
...
if __name__ == '__main__':
  ...
  if args.stage2:
      pseudo_label_ids, margin = get_pseudo_labels(args.train_ids_path, stage1_path_to_anom_scores, args.data_path, margin = args.start_margin, metric = 'centre_mean', current_epoch=TRAIN_PLATEAU_EPOCH, num_pseudo_labels=args.stage2_N, model_name=stages[2] + '/' + model_name_temp)
      ...
      current_epoch, stage2_path_to_anom_scores = dclr_training(args, model_name_temp_stage2, stages[2], pseudo_label_ids= pseudo_label_ids, epochs = args.stage2_epochs, current_epoch = TRAIN_PLATEAU_EPOCH, num_ss=0, self_supervised = 0, semi= 1)
      if args.eval_epoch == 0:
           for key in current_epoch.keys():
               _,_ = dclr_training(args, model_name_temp_stage2, stages[2], pseudo_label_ids= pseudo_label_ids, epochs = current_epoch[key] - TRAIN_PLATEAU_EPOCH, current_epoch = TRAIN_PLATEAU_EPOCH, num_ss=0, self_supervised = 0, semi= 1, seed=int(key))
  ...
  if args.stage3:
        pseudo_label_ids, margin =  get_pseudo_labels(args.train_ids_path, stage2_path_to_anom_scores, args.data_path, margin = args.start_margin, metric = 'w_centre', current_epoch=current_epoch, num_pseudo_labels=263, model_name=stages[3] + '/' + model_name_temp)
        shots = len(pseudo_label_ids)
        ...
        current_epoch, stage3_path_to_anom_scores = dclr_training(args, model_name_temp_stage3, stages[3], pseudo_label_ids = pseudo_label_ids, epochs = args.stage3_epochs, num_ss=0, current_epoch = current_epoch, self_supervised = 0, semi= 1)
        if args.eval_epoch == 0:
              for key in current_epoch.keys():
                  _,_ = dclr_training(args, model_name_temp_stage3, stages[3], pseudo_label_ids = pseudo_label_ids, epochs = current_epoch[key] -  stage2_epoch[key], current_epoch = stage2_epoch[key], num_ss=0, self_supervised = 0, semi= 1, seed=int(key))
  else:
  ...
  if args.stage_severe_pred:
      ...
      current_epoch, stage_severe_path_to_anom_scores = dclr_training(args, model_name_temp_sev, stages[1], pseudo_label_ids= pseudo_label_ids, epochs = args.stage_severe_pred_epochs, current_epoch = TRAIN_PLATEAU_EPOCH, num_ss=0, self_supervised = 0, semi= 1)
      if args.eval_epoch == 0:
           for key in current_epoch.keys():
               _,_ = dclr_training(args,model_name_temp_sev, stages[1], pseudo_label_ids= pseudo_label_ids, epochs = current_epoch[key] - TRAIN_PLATEAU_EPOCH, current_epoch = TRAIN_PLATEAU_EPOCH, num_ss=0, self_supervised = 0, semi= 1)
